...
/** Displays search planet screen.
 * @author Karna Malone
 * @author Daniel Watson
*/
public class SearchPlanetScreen {
	...
	/**
	 * Create search planet window.
	 * @param incomingState Current Game State.
	 */
	public SearchPlanetScreen(Game incomingState) {
	...
	private void finishedWindow() {
		gameState.closePlanetSearch(this);
	}
	...
	private void initialize() {
		...
		JLabel lblCrewMemberTo = new JLabel("Crew Member to Perform the Search");
		lblCrewMemberTo.setForeground(new Color(255, 255, 255));
		lblCrewMemberTo.setFont(new Font("Arial", Font.BOLD, 14));
		
		JComboBox comboBox = new JComboBox();
		comboBox.setModel(new DefaultComboBoxModel(gameState.crew.getCrewList(gameState.getCrewSize())));
		...
		JButton btnSearchAction = new JButton("Search (1 Action)");
		btnSearchAction.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(member.getActions() > 0) {
					gameState.searchPlanet(member);
					frmSpaceExplorer.dispose();
				}else {
					gameState.showPopUp(member.getName()+" doesnt have the required actions to search the planet.", frmSpaceExplorer.getBounds());
				}
			}
		});
